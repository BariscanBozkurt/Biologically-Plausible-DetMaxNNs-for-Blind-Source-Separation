...
pickle_name_for_results = "simulation_results_nnsparse_ablation_MUS_V1.pkl"
...
########################################################################################
########################################################################################
RESULTS_DF = pd.DataFrame(columns = ['trial', 'seed', 'Model', 'MUS', 'SINR', 'SINRlist', 'SNR', 'S', 'A', 'X', 'Wf', 'SNRinp', 'execution_time'])
...
for iter1 in range(NumAverages):
    ...
    for MUS_selection in MUS_list:
        ...
        try: # Try Except for SVD did not converge error
            ...
            WSM_Dict = {'trial' : trial, 'seed' : seed_, 'Model' : 'WSM', 'MUS' : MUS_selection,
