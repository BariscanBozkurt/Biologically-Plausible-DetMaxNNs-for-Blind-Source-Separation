...
class OnlineWSMBSS(BSSBaseClass):
    ...
    def fit_next_antisparse(self, x_current, neural_dynamic_iterations = 750, neural_lr_start = 0.2, neural_lr_stop = 0.05, 
                            return_output = False):
    ...
    def fit_next_nnantisparse(self, x_current, neural_dynamic_iterations = 750, neural_lr_start = 0.2, neural_lr_stop = 0.05, 
                               ...
                               hidden_layer_gain = 10, clip_gain_gradients = True):
        ...
        h,y, oc = self.run_neural_dynamics_nnantisparse_jit(x_current = x_current, h = h, y = y, 
                                                            M_H = M_H, M_Y = M_Y, W_HX = W_HX, W_YH = W_YH, 
                                                            D1 = D1, D2 = D2, beta = beta, zeta = zeta, 
                                                            neural_dynamic_iterations = neural_dynamic_iterations, 
                                                            lr_start = neural_lr_start, lr_stop = neural_lr_stop,
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                if synaptic_lr_rule == "constant":
                    ...
                    # MUS = np.max([gamma_start/(1 + np.log(2 + (i_sample))), gamma_stop])
                    MU_MH = np.max([gammaM_start[0]/(1 + np.log(1 + (i_sample//synaptic_lr_decay_divider))), gammaM_stop[0]])
                    MU_MY = np.max([gammaM_start[1]/(1 + np.log(1 + (i_sample//synaptic_lr_decay_divider))), gammaM_stop[1]])
                    MU_WHX = np.max([gammaW_start[0]/(1 + np.log(1 + (i_sample//synaptic_lr_decay_divider))), gammaW_stop[0]])
                    MU_WYH = np.max([gammaW_start[1]/(1 + np.log(1 + (i_sample//synaptic_lr_decay_divider))), gammaW_stop[1]])
                elif synaptic_lr_rule == "divide_by_index":
